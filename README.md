...
- [ x] Explain benefit(s) using `client-side routing`?
  > Answer: Client side routing loads all of the information in one go, which creates a faster and better overall user experience
- [ x] What does AJAX stand for?
  > Answer: Asynchronous JavaScript and XML
- [ x] What are `controlled components` in React?
  > Answer: A Controlled Component is one that takes its current value through props and notifies changes through callbacks
